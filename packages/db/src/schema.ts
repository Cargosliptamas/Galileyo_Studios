...
export const activeRecordLog = mysqlTable("active_record_log", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const address = mysqlTable("address", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idInvoice: bigint("id_invoice", { mode: "number" }).references(() => invoice.id, { onDelete: "restrict", onUpdate: "restrict" } ),
...
(table) => [
	primaryKey({ columns: [table.id], name: "address_id"}),
]);

export const adminMember = mysqlTable("admin_member", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idAdmin: bigint("id_admin", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idMember: bigint("id_member", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idPlan: bigint("id_plan", { mode: "number" }).references(() => userPlan.id, { onDelete: "cascade", onUpdate: "cascade" } ),
},
(table) => [
...
export const adminMessageLog = mysqlTable("admin_message_log", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
(table) => [
	index("IDX_admin_message_log").on(table.objType, table.objId, table.createdAt),
	primaryKey({ columns: [table.id], name: "admin_message_log_id"}),
]);

export const affiliate = mysqlTable("affiliate", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUserParent: bigint("id_user_parent", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idUserChild: bigint("id_user_child", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
(table) => [
	primaryKey({ columns: [table.id], name: "affiliate_id"}),
]);

export const affiliateInvite = mysqlTable("affiliate_invite", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
(table) => [
	primaryKey({ columns: [table.id], name: "affiliate_invite_id"}),
]);
...
(table) => [
	primaryKey({ columns: [table.id], name: "api_log_id"}),
]);

export const appleAppTransactions = mysqlTable("apple_app_transactions", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const appleNotification = mysqlTable("apple_notification", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const authAssignment = mysqlTable("auth_assignment", {
	itemName: varchar("item_name", { length: 64 }).notNull().references(() => authItem.name, { onDelete: "cascade", onUpdate: "cascade" } ),
	userId: varchar("user_id", { length: 64 }).notNull(),
	createdAt: int("created_at"),
},
(table) => [
	index("idx-auth_assignment-user_id").on(table.userId),
	primaryKey({ columns: [table.itemName, table.userId], name: "auth_assignment_item_name_user_id"}),
]);

export const authItem = mysqlTable("auth_item", {
	name: varchar({ length: 64 }).notNull(),
	type: smallint().notNull(),
	description: text(),
	ruleName: varchar("rule_name", { length: 64 }).references(() => authRule.name, { onDelete: "set null", onUpdate: "cascade" } ),
...
export const authItemChild = mysqlTable("auth_item_child", {
	parent: varchar({ length: 64 }).notNull().references(() => authItem.name, { onDelete: "cascade", onUpdate: "cascade" } ),
	child: varchar({ length: 64 }).notNull().references(() => authItem.name, { onDelete: "cascade", onUpdate: "cascade" } ),
},
(table) => [
	primaryKey({ columns: [table.parent, table.child], name: "auth_item_child_parent_child"}),
]);

export const authRule = mysqlTable("auth_rule", {
	name: varchar({ length: 64 }).notNull(),
	// Warning: Can't parse blob from database
	// blobType: blob("data"),
	createdAt: int("created_at"),
...
(table) => [
	primaryKey({ columns: [table.name], name: "auth_rule_name"}),
]);

export const bpSubscription = mysqlTable("bp_subscription", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
(table) => [
	primaryKey({ columns: [table.id], name: "bp_subscription_id"}),
]);

export const bundle = mysqlTable("bundle", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "bundle_id"}),
]);

export const bundleItem = mysqlTable("bundle_item", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idBundle: bigint("id_bundle", { mode: "number" }).references(() => bundle.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idItem: bigint("id_item", { mode: "number" }).references(() => service.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	type: tinyint(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "bundle_item_id"}),
]);

export const comment = mysqlTable("comment", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idSmsPool: bigint("id_sms_pool", { mode: "number" }).references(() => smsPool.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
(table) => [
	primaryKey({ columns: [table.id], name: "contact_id"}),
]);

export const contractLine = mysqlTable("contract_line", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idService: bigint("id_service", { mode: "number" }).references(() => service.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idSpsContract: bigint("id_sps_contract", { mode: "number" }).references(() => spsContract.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	type: tinyint(),
	payInterval: tinyint("pay_interval"),
	quantity: int(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "contract_line_id"}),
]);

export const contractLinePaid = mysqlTable("contract_line_paid", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idContractLine: bigint("id_contract_line", { mode: "number" }).references(() => contractLine.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idInvoice: bigint("id_invoice", { mode: "number" }).references(() => invoice.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idInvoiceLine: bigint("id_invoice_line", { mode: "number" }).references(() => invoiceLine.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	total: decimal({ precision: 10, scale: 2 }).default('0.00').notNull(),
	// you can use { mode: 'date' }, if you want to have Date as type for this column
	beginAt: date("begin_at", { mode: 'string' }),
	// you can use { mode: 'date' }, if you want to have Date as type for this column
...
(table) => [
	primaryKey({ columns: [table.id], name: "conversation_id"}),
]);

export const conversationFile = mysqlTable("conversation_file", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idConversation: bigint("id_conversation", { mode: "number" }).references(() => conversation.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idConversationMessage: bigint("id_conversation_message", { mode: "number" }).references(() => conversationMessage.id, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const conversationUnviewed = mysqlTable("conversation_unviewed", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }),
	idConversation: bigint("id_conversation", { mode: "number" }).references(() => conversation.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idConversationMessage: bigint("id_conversation_message", { mode: "number" }).references(() => conversationMessage.id, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const conversationUser = mysqlTable("conversation_user", {
	idConversation: bigint("id_conversation", { mode: "number" }).notNull().references(() => conversation.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "restrict", onUpdate: "restrict" } ),
},
(table) => [
	primaryKey({ columns: [table.idConversation, table.idUser], name: "conversation_user_id_conversation_id_user"}),
]);
...
export const creditCard = mysqlTable("credit_card", {
	...
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	updatedAt: datetime("updated_at", { mode: 'string'}),
	anetCustomerPaymentProfileId: varchar("anet_customer_payment_profile_id", { length: 255 }),
...
(table) => [
	primaryKey({ columns: [table.id], name: "credit_card_id"}),
]);
...
(table) => [
	primaryKey({ columns: [table.id], name: "device_id"}),
]);

export const devicePlan = mysqlTable("device_plan", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idDevice: bigint("id_device", { mode: "number" }).references(() => service.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idPlan: bigint("id_plan", { mode: "number" }).references(() => service.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
(table) => [
	primaryKey({ columns: [table.id], name: "device_plan_id"}),
]);
...
export const emailPoolAttachment = mysqlTable("email_pool_attachment", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idEmailPool: bigint("id_email_pool", { mode: "number" }).notNull().references(() => emailPool.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
export const emergencyTipsRequest = mysqlTable("emergency_tips_request", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const follower = mysqlTable("follower", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idFollowerList: bigint("id_follower_list", { mode: "number" }).notNull().references(() => followerList.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idUserLeader: bigint("id_user_leader", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idUserFollower: bigint("id_user_follower", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	isActive: tinyint("is_active"),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	updatedAt: datetime("updated_at", { mode: 'string'}),
...
(table) => [
	primaryKey({ columns: [table.id], name: "follower_id"}),
]);

export const followerList = mysqlTable("follower_list", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	name: varchar({ length: 255 }).notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "follower_list_id"}),
]);

export const iexWebhook = mysqlTable("iex_webhook", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "iex_webhook_id"}),
]);

export const influencerAssistant = mysqlTable("influencer_assistant", {
	idInfluencer: bigint("id_influencer", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idAssistant: bigint("id_assistant", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	updatedAt: datetime("updated_at", { mode: 'string'}),
},
(table) => [
	primaryKey({ columns: [table.idInfluencer, table.idAssistant], name: "influencer_assistant_id_influencer_id_assistant"}),
]);

export const influencerPage = mysqlTable("influencer_page", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idSubscription: bigint("id_subscription", { mode: "number" }).notNull().references(() => subscription.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
export const infoState = mysqlTable("info_state", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	key: varchar({ length: 255 }).notNull(),
	value: json(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "invite_id"}),
]);

export const inviteAffiliate = mysqlTable("invite_affiliate", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idInviter: bigint("id_inviter", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idInvited: bigint("id_invited", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idInviteInvoice: bigint("id_invite_invoice", { mode: "number" }).notNull().references(() => invoice.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idRewardInvoice: bigint("id_reward_invoice", { mode: "number" }).references(() => invoice.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
},
(table) => [
	primaryKey({ columns: [table.id], name: "invite_affiliate_id"}),
]);

export const invoice = mysqlTable("invoice", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idBpSubscribe: bigint("id_bp_subscribe", { mode: "number" }).references(() => bpSubscription.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
(table) => [
	primaryKey({ columns: [table.id], name: "invoice_id"}),
]);

export const invoiceLine: any = mysqlTable("invoice_line", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idInvoice: bigint("id_invoice", { mode: "number" }).references(() => invoice.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	type: tinyint().default(0).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "invoice_line_id"}),
]);

export const invoicePromocode = mysqlTable("invoice_promocode", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idPromo: bigint("id_promo", { mode: "number" }).notNull().references(() => promocode.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idInvoice: bigint("id_invoice", { mode: "number" }).notNull().references(() => invoice.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
export const invoiceService = mysqlTable("invoice_service", {
	idInvoice: bigint("id_invoice", { mode: "number" }).notNull().references(() => invoice.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idService: bigint("id_service", { mode: "number" }).notNull().references(() => service.id, { onDelete: "cascade", onUpdate: "cascade" } ),
},
(table) => [
	index("IDX_invoice_service").on(table.idInvoice, table.idService),
]);

export const logAuthorize = mysqlTable("log_authorize", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idMoneyTransaction: bigint("id_money_transaction", { mode: "number" }).references(() => moneyTransaction.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	name: varchar({ length: 255 }),
	request: json(),
	response: json(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "log_authorize_id"}),
]);
...
export const loginStatistic = mysqlTable("login_statistic", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
(table) => [
	primaryKey({ columns: [table.id], name: "login_statistic_id"}),
]);

export const marketstackIndx = mysqlTable("marketstack_indx", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	name: varchar({ length: 64 }).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "marketstack_indx_id"}),
]);

export const memberRequest = mysqlTable("member_request", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idUserFrom: bigint("id_user_from", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	type: tinyint(),
	text: text(),
	params: json(),
	isActive: tinyint("is_active").default(1).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "member_request_id"}),
]);

export const memberTemplate = mysqlTable("member_template", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idAdmin: bigint("id_admin", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	firstName: varchar("first_name", { length: 50 }),
	lastName: varchar("last_name", { length: 255 }),
...
(table) => [
	primaryKey({ columns: [table.id], name: "member_template_id"}),
]);

export const migration = mysqlTable("migration", {
...
export const moneyTransaction = mysqlTable("money_transaction", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idInvoice: bigint("id_invoice", { mode: "number" }).references(() => invoice.id),
	idCreditCard: bigint("id_credit_card", { mode: "number" }).references(() => creditCard.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
export const newsContent = mysqlTable("news_content", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idNews: bigint("id_news", { mode: "number" }).references(() => news.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	status: tinyint().default(0).notNull(),
	params: json(),
	content: longtext(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "news_content_id"}),
]);
...
export const pageContent = mysqlTable("page_content", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idPage: bigint("id_page", { mode: "number" }).references(() => page.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	status: tinyint().default(0).notNull(),
	params: json(),
	content: longtext(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "page_content_id"}),
]);

export const phoneNumber = mysqlTable("phone_number", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idProvider: bigint("id_provider", { mode: "number" }).references(() => provider.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
(table) => [
	primaryKey({ columns: [table.id], name: "phone_number_id"}),
]);
...
(table) => [
	primaryKey({ columns: [table.id], name: "podcast_id"}),
]);

export const productPhoto = mysqlTable("product_photo", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idService: bigint("id_service", { mode: "number" }).references(() => service.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	folderName: varchar("folder_name", { length: 255 }),
	sizes: json(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "product_photo_id"}),
]);

export const promocode = mysqlTable("promocode", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	type: tinyint().notNull(),
...
export const promocodeInfluencer = mysqlTable("promocode_influencer", {
	idPromocode: bigint("id_promocode", { mode: "number" }).notNull().references(() => promocode.id, { onDelete: "restrict", onUpdate: "restrict" } ),
},
(table) => [
	primaryKey({ columns: [table.idPromocode], name: "promocode_influencer_id_promocode"}),
]);
...
export const providerTwilioCarrier = mysqlTable("provider_twilio_carrier", {
	idProvider: bigint("id_provider", { mode: "number" }).notNull().references(() => provider.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idTwilioCarrier: bigint("id_twilio_carrier", { mode: "number" }).notNull().references(() => twilioCarrier.id, { onDelete: "cascade", onUpdate: "cascade" } ),
});

export const reaction = mysqlTable("reaction", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const recentSearch = mysqlTable("recent_search", {
...
(table) => [
	primaryKey({ columns: [table.id], name: "recent_search_id"}),
]);
...
(table) => [
	primaryKey({ columns: [table.id], name: "register_id"}),
]);

export const reportArchive = mysqlTable("report_archive", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "report_archive_id"}),
]);

export const reportReferral = mysqlTable("report_referral", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const service = mysqlTable("service", {
	...
	specialFeeAnnual: decimal("special_fee_annual", { precision: 10, scale: 2 }),
	isSpecialFeeAnnual: tinyint("is_special_fee_annual").default(0).notNull(),
	specialDatetime: datetime("special_datetime", { mode: 'string'}),
},
(table) => [
	primaryKey({ columns: [table.id], name: "service_id"}),
]);

export const serviceCustom = mysqlTable("service_custom", {
	id: int().autoincrement().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "service_custom_id"}),
]);
...
(table) => [
	primaryKey({ columns: [table.name], name: "settings_name"}),
]);

export const settingsSafe = mysqlTable("settings_safe", {
...
export const smsPool = mysqlTable("sms_pool", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idStaff: bigint("id_staff", { mode: "number" }).references(() => staff.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	...
	idAssistant: bigint("id_assistant", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	shortBody: varchar("short_body", { length: 160 }),
	url: varchar({ length: 512 }),
	isBan: tinyint("is_ban").default(0).notNull(),
},
(table) => [
	primaryKey({ columns: [table.id], name: "sms_pool_id"}),
]);

export const smsPoolPhoneNumber = mysqlTable("sms_pool_phone_number", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idSmsPool: bigint("id_sms_pool", { mode: "number" }).references(() => smsPool.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idPhoneNumber: bigint("id_phone_number", { mode: "number" }).references(() => phoneNumber.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
export const smsPoolPhoto = mysqlTable("sms_pool_photo", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idSmsPool: bigint("id_sms_pool", { mode: "number" }).references(() => smsPool.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idSmsShedule: bigint("id_sms_shedule", { mode: "number" }).references(() => smsShedule.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	folderName: varchar("folder_name", { length: 255 }),
	webName: varchar("web_name", { length: 255 }),
	sizes: json(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "sms_pool_photo_id"}),
]);

export const smsPoolReaction = mysqlTable("sms_pool_reaction", {
	idSmsPool: bigint("id_sms_pool", { mode: "number" }).notNull().references(() => smsPool.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idReaction: bigint("id_reaction", { mode: "number" }).references(() => reaction.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
},
(table) => [
	primaryKey({ columns: [table.idSmsPool, table.idUser], name: "sms_pool_reaction_id_sms_pool_id_user"}),
]);

export const smsPoolReport = mysqlTable("sms_pool_report", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "sms_pool_report_id"}),
]);

export const smsShedule = mysqlTable("sms_shedule", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idStaff: bigint("id_staff", { mode: "number" }).references(() => staff.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idSubscription: bigint("id_subscription", { mode: "number" }).references(() => subscription.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idFollowerList: bigint("id_follower_list", { mode: "number" }).references(() => followerList.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idSmsPool: bigint("id_sms_pool", { mode: "number" }).references(() => smsPool.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	purpose: tinyint(),
	status: tinyint().default(0),
	body: text().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "sms_shedule_id"}),
]);

export const spsAddUserRequest = mysqlTable("sps_add_user_request", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	token: varchar({ length: 255 }),
...
export const spsContract = mysqlTable("sps_contract", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
export const subscription = mysqlTable("subscription", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idSubscriptionCategory: bigint("id_subscription_category", { mode: "number" }).notNull().references(() => subscriptionCategory.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	name: varchar({ length: 255 }).notNull(),
	title: varchar({ length: 255 }),
	...
	idInfluencer: bigint("id_influencer", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	price: decimal({ precision: 10, scale: 2 }).default('0.00').notNull(),
	bonusPoint: int("bonus_point").default(0).notNull(),
	token: varchar({ length: 255 }),
...
export const subscriptionCategory = mysqlTable("subscription_category", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	name: varchar({ length: 255 }).notNull(),
	idParent: bigint("id_parent", { mode: "number" }),
...
export const subscriptionWizard = mysqlTable("subscription_wizard", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	settings: json(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
...
export const twilioCarrier = mysqlTable("twilio_carrier", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	name: varchar({ length: 255 }).notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
},
...
export const twilioIncoming = mysqlTable("twilio_incoming", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	number: varchar({ length: 255 }),
	body: text(),
	message: json(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "twilio_incoming_id"}),
]);
...
export const user = mysqlTable("user", {
	...
	anetCustomerShippingAddressId: varchar("anet_customer_shipping_address_id", { length: 255 }),
	bonusPoint: int("bonus_point").default(0).notNull(),
	image: varchar({ length: 63 }),
...
export const userFollowerAlert = mysqlTable("user_follower_alert", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	total: int().default(0),
	used: int().default(0),
...
export const userFriend = mysqlTable("user_friend", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idFriend: bigint("id_friend", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
},
(table) => [
...
export const userPlan = mysqlTable("user_plan", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idService: bigint("id_service", { mode: "number" }).references(() => service.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idInvoiceLine: bigint("id_invoice_line", { mode: "number" }).references(() => invoiceLine.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	isPrimary: tinyint("is_primary").default(1).notNull(),
	alert: int().default(0),
	maxPhoneCnt: int("max_phone_cnt"),
	payInterval: tinyint("pay_interval"),
	priceBeforeProrate: decimal("price_before_prorate", { precision: 10, scale: 2 }),
	priceAfterProrate: decimal("price_after_prorate", { precision: 10, scale: 2 }),
	settings: json(),
	beginAt: datetime("begin_at", { mode: 'string'}).notNull(),
	endAt: datetime("end_at", { mode: 'string'}),
...
(table) => [
	primaryKey({ columns: [table.id], name: "user_plan_id"}),
]);

export const userPlanShedule = mysqlTable("user_plan_shedule", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).references(() => user.id, { onDelete: "cascade" } ),
	idService: bigint("id_service", { mode: "number" }).references(() => service.id),
	settings: json(),
...
export const userPointHistory = mysqlTable("user_point_history", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idUserPointSettings: bigint("id_user_point_settings", { mode: "number" }).notNull().references(() => userPointSettings.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idSmsPool: bigint("id_sms_pool", { mode: "number" }).references(() => smsPool.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	idComment: bigint("id_comment", { mode: "number" }).references(() => comment.id, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const userPointSettings = mysqlTable("user_point_settings", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	title: varchar({ length: 255 }),
	price: int().notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
...
export const userSatelliteSubscription = mysqlTable("user_satellite_subscription", {
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idSubscription: bigint("id_subscription", { mode: "number" }).notNull().references(() => subscription.id, { onDelete: "cascade", onUpdate: "cascade" } ),
});
...
export const userSubscriptionAddress = mysqlTable("user_subscription_address", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	idUser: bigint("id_user", { mode: "number" }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	idSubscription: bigint("id_subscription", { mode: "number" }).notNull().references(() => subscription.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	zip: varchar({ length: 10 }),
...
export const weatherGovProcess = mysqlTable("weather_gov_process", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const zipUs = mysqlTable("zip_us", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	...
	daylightSavingsTimeFlag: varchar("daylight_savings_time_flag", { length: 1 }),
},
(table) => [
