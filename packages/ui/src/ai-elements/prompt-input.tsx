...
import type { ChatStatus, FileUIPart as FileUIPartAi } from "ai";
import type {
  ChangeEventHandler,
  ...
  return (
    ...
    >
      ...
      <Button
        ...
        className="absolute right-0 top-0 h-6 w-6 rounded-full opacity-0 group-hover:opacity-100"
...
export const PromptInput = ({
  ...
  const matchesAccept = useCallback(
    (f: File) => {
      ...
      const validators = accept.split(",").map((type) => type.trim());

      if (
        validators.length > 0 &&
        validators.some((validator) =>
          f.type.startsWith(validator.replace("/*", "")),
        )
      ) {
        return true;
      }
