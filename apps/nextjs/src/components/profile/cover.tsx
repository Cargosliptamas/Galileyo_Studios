...
import { Loader2, Trash2, Upload, ArrowLeftIcon, ZoomInIcon, ZoomOutIcon, Image as ImageIcon } from "lucide-react";

import { Button } from "@galileyo/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@galileyo/ui/card";
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from "@galileyo/ui/dialog";
import { Cropper, CropperCropArea, CropperDescription, CropperImage } from "@galileyo/ui/cropper";
...
export function Cover() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      formData.append("header_file", new File([croppedBlob], "cover-image.jpg"))
      ...
      <CardContent className="space-y-4">
        <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-slate-100/50 dark:border-slate-700 dark:bg-slate-800/50">
          ...
          <div className="absolute bottom-3 right-3 flex gap-2">
            <div
              ...
              aria-label={finalImageUrl ? "Change cover image" : "Upload cover image"}
            >
            ...
            <Button variant="destructive" onClick={() => removeHeaderPicture.mutate()} disabled={removeHeaderPicture.isPending}>
            ...
            <input {...getInputProps()} className="sr-only" aria-label="Upload cover image" tabIndex={-1} />
          </div>
        </div>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="gap-0 p-0 sm:max-w-140 *:[button]:hidden">
            ...
            <DialogHeader className="contents space-y-0 text-left">
              <DialogTitle className="flex items-center justify-between border-b p-4 text-base">
                <div className="flex items-center gap-2">
                  <Button type="button" variant="ghost" size="icon" className="-my-1 opacity-60" onClick={() => setIsDialogOpen(false)} aria-label="Cancel">
            ...
            {previewUrl && (
              <Cropper className="h-96 sm:h-120" image={previewUrl} zoom={zoom} onCropChange={handleCropChange} onZoomChange={setZoom} aspectRatio={16/9}>
                <CropperDescription />
                <CropperImage />
                <CropperCropArea />
              </Cropper>
            ...
            <DialogFooter className="border-t px-4 py-6">
              <div className="mx-auto flex w-full max-w-80 items-center gap-4">
                ...
                <Slider defaultValue={[1]} value={[zoom]} min={1} max={3} step={0.1} onValueChange={(value) => setZoom(value[0] ?? 1)} aria-label="Zoom slider" />
